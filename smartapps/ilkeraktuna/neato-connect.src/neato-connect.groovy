	...
	return dynamicPage(name: "smartSchedulePAGE", title: "SmartSchedule for ${state.botvacDevices[botvacId]}", install: false, uninstall: false) { 
    	section() {
            ...
            if (settings["smartScheduleEnabled#$botvacId"]) {
                ...
                paragraph "Routine override open doors will cancel the next scheduled clean and reset the interval countdown."
                	input ("ssOverrideDoor#$botvacId", "capability.contactSensor", title:"Set SmartSchedule override doors", multiple: true, required: false)
                    input ("ssOverrideDoorCondition#$botvacId", "enum", title:"Override schedule when:", multiple: false, required: true, options: ["any": "Any door open", "all": "All doors open"], defaultValue: "any") 
                }
                section("Notifications:") {
...
def getSmartScheduleString(botvacId) {
    ...
    if (settings["smartScheduleEnabled#$botvacId"]) {
        ...
        if (settings["ssOverrideDoor#$botvacId"]) {
        	if (settings["ssOverrideDoorCondition#$botvacId"] == "any") {
            	listString += "• Override schedule if any of ${settings["ssOverrideDoor#$botvacId"]} are open."
            } else {
        		listString += "• Override schedule if all ${settings["ssOverrideDoor#$botvacId"]} are open."
            }
        }
    }
    return listString
...
def smartScheduleHandler(evt) {
    ...
    getChildDevices().each { childDevice ->
        ...
        def doorInList = false
    		for (doorName in settings["ssOverrideDoor#$botvacId"]) {
        		if (doorName.currentValue("contact") == "open") {
            		doorInList = true
            		break
            	}
        	}
    		if (doorInList) {
    			def executeOverride1 = true
        		//If override switch condition is ALL...
    			if (settings["ssOverrideDoorCondition#$botvacId"] == "all") {
        			//Check all doors in override door settings are on
            		for (doorVal in settings["ssOverrideDoor#$botvacId"].currentValue("contact")) {
        				if (doorVal == "closed") {
            				executeOverride1 = false
            				break
        				}
    				}
        		}
              if (executeOverride1) {
        		...
        		if (settings.ssNotification) {
        			messageHandler("Neato SmartSchedule has reset schedule for ${childDevice.name} as override door is open.", false)
        		}
        	}
